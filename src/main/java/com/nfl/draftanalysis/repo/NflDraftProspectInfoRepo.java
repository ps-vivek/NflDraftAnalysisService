...
public interface NflDraftProspectInfoRepo extends JpaRepository<NflDraftProspectInfo, Long> {
	...
	@Query("select t from NflDraftProspectInfo t where t.year=:year and t.team in (:team) and t.draftedRound in (:draftedRound) and t.status is not null order by t.team")
